...
class GeneratorJava extends Generator {
    ...
    public void generate(Writer writer) throws IOException, SyntaxException {
        ...
        if(grammar.getParserPackage() != null) {
            writeLine(writer, "package " + grammar.getParserPackage() + ";");
            writeLine(writer, "");
        }
        ...
        writeLine(writer, "");
        writeLine(writer, "public class " + grammar.getParserClass() + " {");
        writeLine(writer, "");
        writeLine(writer, "    private " + grammar.getInputType() + "[] input;");
        writeLine(writer, "");
        ...
        for(SymbolNonTerm sym : grammar.getExports()) {
            writeLine(writer, "");
            writeLine(writer, "    public " + sym.getType() + " parse" + sym.getName() + "(String input) throws ParserException {");
            writeLine(writer, "        this.input = input.toCharArray();");
            writeLine(writer, "        Ref_int pos = new Ref_int(0);");
            writeLine(writer, "        Ref<" + sym.getType() + "> output = new Ref<>(null);");
            writeLine(writer, "        if(nt_" + sym.getName() + "(pos, output) && pos.val == this.input.length) { ");
        ...
        if(need_ts) {
            writeLine(writer, "");
            writeLine(writer, "    private boolean ts(final Ref_int pos, " + grammar.getInputType() + "[] s) {");
            writeLine(writer, "        for(" + grammar.getInputType() + " c : s) {");
            writeLine(writer, "            if(pos.val >= input.length || input[pos.val] != c) return false;");
        ...
        if(need_tc) {
            writeLine(writer, "");
            writeLine(writer, "    private boolean tc(final Ref_int pos, " + grammar.getInputType() + " c) {");
            writeLine(writer, "        if(pos.val >= input.length || input[pos.val] != c) return false;");
            writeLine(writer, "        pos.val++;");
            writeLine(writer, "        return true;");
            writeLine(writer, "    }");
        ...
        if(need_tset) {
            writeLine(writer, "");
            writeLine(writer, "    private boolean tset(final Ref_int pos, " + grammar.getInputType() + "[] s) {");
            writeLine(writer, "        for(int i = 0; i < s.length; i++) {");
            writeLine(writer, "            if(pos.val < input.length && s[i] == input[pos.val]) {");
            writeLine(writer, "                pos.val++;");
            writeLine(writer, "                return true;");
            writeLine(writer, "            }");
            writeLine(writer, "        }");
        ...
        if(need_trange) {
            writeLine(writer, "");
            writeLine(writer, "    private boolean trange(final Ref_int pos, " + grammar.getInputType() + " c1, " + grammar.getInputType() + " c2) {");
            writeLine(writer, "        if(pos.val >= input.length || input[pos.val] < c1 || input[pos.val] > c2) return false;");
            writeLine(writer, "        pos.val++;");
            writeLine(writer, "        return true;");
            writeLine(writer, "    }");
        }
        if(need_tnotset) {
            writeLine(writer, "");
            writeLine(writer, "    private boolean tnotset(final Ref_int pos, " + grammar.getInputType() + "[] s) {");
            writeLine(writer, "        for(int i = 0; i < s.length; i++) {");
            writeLine(writer, "            if(pos.val >= input.length || s[i] == input[pos.val]) {");
            writeLine(writer, "                return false;");
            writeLine(writer, "            }");
        ...
        for (int i = 0; i < literals.size(); i++) {
            writeLine(writer, "    private static final " + grammar.getInputType() + "[] t_" + (i+1) + " = \"" + quote(literals.get(i)) + "\".toCharArray();");
        }
        writeLine(writer, "");
