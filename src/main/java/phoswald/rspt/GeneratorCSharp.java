...
class GeneratorCSharp extends Generator {

    GeneratorCSharp(Grammar grammar) {
        super(grammar);
    }
    ...
    public void generate(Writer writer) throws IOException, SyntaxException {
        ...
        writeLine(writer, "");
        writeLine(writer, "public partial class " + grammar.getParserClass() + " {");
        writeLine(writer, "");
        writeLine(writer, "    private " + grammar.getInputType() + "[] _input;");
        writeLine(writer, "");
        writeLine(writer, "    public " + grammar.getParserClass() + "() { }");
        for(SymbolNonTerm sym : grammar.getExports()) {
            writeLine(writer, "");
            writeLine(writer, "    public bool Parse_" + sym.getName() + "(" + grammar.getInputType() + "[] input, out " + sym.getType() + " output, out int pos) {");
        ...
        if(need_ts) {
            writeLine(writer, "");
            writeLine(writer, "    private bool ts(ref int pos, " + grammar.getInputType() + "[] s) {");
            writeLine(writer, "        foreach(" + grammar.getInputType() + " c in s) {");
        ...
        if(need_tc) {
            writeLine(writer, "");
            writeLine(writer, "    private bool tc(ref int pos, " + grammar.getInputType() + " c) {");
            writeLine(writer, "        if(pos >= _input.Length || _input[pos] != c) return false;");
            writeLine(writer, "        pos++;");
            writeLine(writer, "        return true;");
            writeLine(writer, "    }");
        ...
        if(need_tset) {
            writeLine(writer, "");
            writeLine(writer, "    private bool tset(ref int pos, " + grammar.getInputType() + "[] s) {");
        ...
        if(need_trange) {
            writeLine(writer, "");
            writeLine(writer, "    private bool trange(ref int pos, " + grammar.getInputType() + " c1, " + grammar.getInputType() + " c2) {");
        ...
        if(need_tnotset) {
            writeLine(writer, "");
            writeLine(writer, "    private bool tnotset(ref int pos, " + grammar.getInputType() + "[] s) {");
            writeLine(writer, "        for(int i = 0; i < s.Length; i++) {");
            writeLine(writer, "            if(pos >= _input.Length || s[i] == _input[pos]) {");
            writeLine(writer, "                return false;");
            writeLine(writer, "            }");
